...
function showStimulus(p)
    ...
    if p.trial.stimulus.frameI<=p.trial.stimulus.nrFrames
        ...
        noisevec = p.trial.stimulus.noisevec;
        ...
        %if lifetime is expired, update direction
        if p.trial.stimulus.dotLifeFr>0
            idx=find(lifetime==0);

            %find proportion of noise and signal dots to keep numbers fixed
            nrSignal=sum(noisevec(idx)); %1 = signal dot
            rvec=zeros(size(idx));
            rvec(1:nrSignal)=1;
            rvec=rvec(randperm(length(rvec)));
            
            for i=1:length(idx)
                if rvec(i)==1 %signal dot, move with stimulus direction
