function s = lesion_dots_settings_P8_XXXX0
%post-lesion testing
...
s.stimulus.delta_coh = 0.1;%step size for offset in deg
s.stimulus.dotCoherence = .5;
...
s.stimulus.centerY=860; %vertical stimulus position (pixels); middle =540, bottom =860, top =220
s.stimulus.offset=15; %horizontal offset in deg (from point where stimulus turns on)
%s.datapixx.din.channels.ports = [0 2 4 6 10];
%s.ports.nPorts = 5;

s.stimulus.iniMatchType=0; %value:  0-normal, 1-non-matching choice, 2-matching choice
s.stimulus.cond.Ncond=4; %2 sides x 2 dir
